...
const civids = (x: number) => {
    ...
    const color = colors[Math.floor(x * (colors.length - 1))]
    return "rgb(" + color.map((c: number) => Math.round(c * 255)).join(",") + ")"
}
...
export const LockedModePlot = ({ data, plotId: externalId, lockedModeCategory }: LockedModePlotProps) => {
    ...
    useEffect(() => {
        ...
        // const vMin = Math.min(...ampl.current)
        // const vMax = Math.max(...ampl.current)
        const vMin = 1E-4
        const vMax = 1E-3
