...
export const TimeSeriesProvider = ({
  ...
  // Binds annotations created here to a signal. Plots that annotate a single implicit
  // signal leave this null; profile 2D passes the signal from its view params.
  ...
  // Annotation types this context cannot represent as points (masks, class labels,
  // video annotations). They are held here untouched and re-attached on sync so that
  // editing time series annotations does not delete them from the sample.
  const unmanagedAnnotations = useRef<Annotation[]>([]);
